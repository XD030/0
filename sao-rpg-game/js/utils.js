...
/* ════════════════ 顯示名稱 helper(Task B Patch)════════════════
 * 取顯示名稱:命名武器(customName 非 null/空字串)優先,否則 fallback def name。
 * 適用 bag instance(weapon/armor)、equipment slot snapshot、市集/商店 sell list 物件。
 * 防呆:傳 null/undefined 回空字串。
 * armor 目前沒命名系統 → customName 永遠 falsy → 自動 fallback,不用分支判斷。
 */
...
/* ════════════════ 篩選列水平拖曳 / 滾輪滾動(桌面用) ════════════════
 * 行動端用 touch-pan 自然 scroll,桌面滑鼠沒對應行為,故加此 helper。
 * 綁在外層穩定容器上,內層 .bag-filter-row-inner 重建不掉 listener。
 * 用 capture-phase click 攔截解決 drag vs click 衝突。
 */
function attachDragScroll(outerEl){
  if(!outerEl || outerEl._dragScrollAttached) return;
  outerEl._dragScrollAttached = true;

  let isDown=false, startX=0, startScroll=0, target=null, moved=false;

  outerEl.addEventListener('mousedown', e=>{
    const inner = e.target.closest('.bag-filter-row-inner');
    if(!inner) return;
    isDown=true; moved=false; target=inner;
    startX=e.clientX; startScroll=inner.scrollLeft;
  });
  ...
  const stop = ()=>{ isDown=false; target=null; };
  outerEl.addEventListener('mouseup', stop);
  outerEl.addEventListener('mouseleave', stop);
  ...
  outerEl.addEventListener('wheel', e=>{
    const inner = outerEl.querySelector('.bag-filter-row-inner');
    if(!inner) return;
