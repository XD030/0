...
(function bootstrap(){
  ...
  if(typeof isHuntRunning==='function' && isHuntRunning()){
    // updateLifeSkillLocks 已隨架構搬遷刪除;狩獵鎖在 goLifeSkillPage 入口處理
    // isHuntRunning 重連邏輯保留(其他地方還有用)
  ...
  // 8. 篩選列桌面拖曳 / 滾輪 → 水平 scroll(行動端維持原有 touch-pan)
  ['bag-filter-row','market-filter-row','shop-filter-row'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) attachDragScroll(el);
  });
})();
