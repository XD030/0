...
function migrateEquipVal(key, val){
  ...
  if(w) return {src:'bag', uid:w.uid, name:w.name, rarity:w.rarity, stat:w.statStr||'', durability:w.dur, maxDurability:w.maxDur};
  const a=(bag.armors||[]).find(x=>x.uid===val);
  if(a) return {src:'bag', uid:a.uid, name:a.name, rarity:a.rarity, stat:a.statStr||'', durability:a.dur, maxDurability:a.maxDur};
...
function equipFromBag(type, uid){
  ...
  if(!item){ showToast('// 找不到裝備'); return; }
  s.equipment[slotKey]={
    src:'bag', uid:item.uid, name:item.name, rarity:item.rarity,
    stat:item.statStr||'', durability:item.dur, maxDurability:item.maxDur,
  };
  save(s);
  showToast(`// 裝備 ${item.name}`);
...
function unequipItem(slotKey){
  ...
  const name=s.equipment[slotKey].name||'裝備';
...
function openEquipDD(key, name, cb){
  ...
  bagItems.forEach(item=>{
    ...
    d.innerHTML=`<span style="color:#ddd">${item.name}</span>${durStr}<span class="dropdown-rarity r-${item.rarity}">${(item.rarity||'').toUpperCase()}</span>`;
    d.onclick=()=>{
      s.equipment[key]={
        src:'bag', uid:item.uid, name:item.name, rarity:item.rarity,
        stat:item.statStr||'', durability:item.dur, maxDurability:item.maxDur,
